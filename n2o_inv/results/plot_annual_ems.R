...
regional_ems_plot <- function(annual_flux_samples, region, NAME_COLOURS, labels, first_year, last_year) {
  ...
  if (region == "Global") {
    ...
    p <- plot_annual_ems(regional_flux_samples, NAME_COLOURS, labels, first_year, last_year)
  } else {
...
main <- function() {
  ...
  p_global <- regional_ems_plot(annual_flux_samples, "Global", NAME_COLOURS, labels, first_year, last_year)
  ...
  regional_plots <- lapply(sprintf("T%02d", seq(0, max_land_region)), regional_ems_plot,
                           annual_flux_samples=annual_flux_samples, NAME_COLOURS=NAME_COLOURS, labels=labels, first_year=first_year, last_year=last_year)
  ...
  regional_plots <- lapply(sprintf("T%02d", seq(max_land_region + 1, config$inversion_constants$no_regions)),
                          regional_ems_plot, annual_flux_samples=annual_flux_samples, NAME_COLOURS=NAME_COLOURS, labels=labels, first_year=first_year, last_year=last_year)
  p_regional <- ggarrange(plotlist=regional_plots, ncol=4, nrow=3, common.legend = TRUE, legend="bottom")
  p_regional <- annotate_figure(p_regional, left = textGrob(expression(N[2] * "O Flux / TgN " * yr^-1),
                                                            rot = 90, vjust = 0.5, gp = gpar(cex = 1.3)))
