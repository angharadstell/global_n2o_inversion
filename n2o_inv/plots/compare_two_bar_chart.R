...
compare_two <- function(case1, case2, case3, string_pattern, title, labels) {
    ...
    case2_annual_flux_samples <- get_annual_ems(case2) %>%
                                   mutate(case = case2) %>%
                                   filter(year == 2011, estimate == "Posterior")
    case3_annual_flux_samples <- get_annual_ems(case3) %>%
                                   mutate(case = case3) %>%
                                   filter(year == 2011, estimate == "Posterior")
    ...
    annual_flux_samples <- bind_rows(case1_annual_flux_samples %>% filter(grepl(string_pattern, name)),
                                     ...
                                     case3_annual_flux_samples %>% filter(grepl(string_pattern, name)))

    # make case a factorto keep order in bar chart
    annual_flux_samples$case <- factor(annual_flux_samples$case,
                                       levels = rev(unique(annual_flux_samples$case)))
...
p_land <- compare_two(paste0(base_case, "-rescaled-halfland_window01"),
                      paste0(base_case, "_window01"),
                      paste0(base_case, "-rescaled-doubleland_window01"),
                      "^Global",
                      NULL,
                      c("half prior", "prior", "double prior"))
