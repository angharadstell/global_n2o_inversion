...
def create_obspack_id(site, year, month, day, identifier):
    ...
    # make the string 200 characters
    whole_string.ljust(200, " ")
    return whole_string.encode()#.ljust(200, " ")
...
with xr.open_dataset("/work/as16992/geoschem/N2O/obs/obspack_n2o_noaasurface.20150101.nc") as load:
    noaasurface = load.load() 
...
for site in sites:
    ...
    agage_obs[site][0] = agage_obs[site][0].drop(["status_flag", "integration_flag"])
    ...
    #agage_obs[site][0]["CT_sampling_strategy"] = (("obs"), np.array([4.] * no_obs))
