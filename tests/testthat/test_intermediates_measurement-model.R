...
args$process_model <- paste0(config$paths$geos_inte, "/process-model-arranged-time.rds")
...
measurement_model <- flux_measurement_model(
  ...
  matching = 1:6116,#match(observations$observation_id, process_model$control_mole_fraction$observation_id),
...
C_arranged <- measurement_model$C